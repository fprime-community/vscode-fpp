...
pub struct BuildSpecLocMap {
    scope: RefCell<Vec<String>>,
}

impl Default for BuildSpecLocMap {
    fn default() -> Self {
        Self::new()
    }
}

impl BuildSpecLocMap {
    pub fn new() -> BuildSpecLocMap {
        BuildSpecLocMap {
...
impl<'ast> Visitor<'ast> for BuildSpecLocMap {
    ...
    fn visit_def_module(
        ...
        self.scope.borrow_mut().push(node.name.data.clone());
    ...
    fn visit_spec_loc(&self, a: &mut Self::State, node: &'ast SpecLoc) -> ControlFlow<Self::Break> {
        let mut parts: Vec<String> = self.scope.borrow().clone();
