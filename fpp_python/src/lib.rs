...
//! files. The recording walk and the AST-node wrappers come from
//! `fpp_python_macros::fpp_ast_bindings!` over the `crate::ast` declaration
//! (`src/ast/defs.rs`, regenerated by
//! `cargo run -p fpp_python --features bindgen --bin fpp_bindgen`). The
//! semantic-layer wrappers (`crate::sem`) come from
//! `fpp_python_macros::fpp_sem_bindings!` over the single generated
//! `src/sem/defs.rs` (a strict 1:1 mirror of `fpp_analysis::Analysis`,
//! regenerated by
//! `cargo run -p fpp_python --features bindgen --bin fpp_sem_bindgen`), with
//! `src/sem/hand.rs` supplying the one escape hatch the macro cannot produce
//! mechanically (`build_type`). Everything else is the
