...
) : ComboBox<LspVersionComboBox.Item>() {
        ...
        renderer = listCellRenderer {
            ...
            value.let {
                if (it is Item.InstalledVersion && it.index == 0) {
                    separator {
                        text = FppBundle.message("fpp.settings.lsp.version.combobox.installed")
                    }
                } else if (it is Item.VersionForDownload && it.index == 0) {
                    separator {
                        text = FppBundle.message("fpp.settings.lsp.version.combobox.download")
                    }
                }
            }
            text(value.text) {
