...
module AnnotatedModule {
  ...
  enum Status {
    @ Active status
    ACTIVE = 0
    @< Post-annotation for ACTIVE
    # Comment between enum members
    @ Idle status
    IDLE = 1
    @< Post-annotation for IDLE
    STOPPED # inline comment
  } default IDLE
  @< Post-annotation for enum

  # Comment before type
  @ Type alias annotation
  type MyType = U32
  @< Post-annotation for type

  @ Array with annotations
  array DataArray = [50] F32 default 0.0
  @< Post-annotation for array
